...
from src.utils.functions_utils import (
    set_seed,
    get_model_path_list,
...
def train_base(opt, train_examples, dev_examples=None):
    with open(
        os.path.join(opt.mid_data_dir, f"{opt.task_type}_ent2id.json"), encoding="utf-8"
    ) as f:
        ent2id = json.load(f)

    train_features = convert_examples_to_features(
        opt.task_type, train_examples, opt.max_seq_len, opt.bert_dir, ent2id
    )[0]
    ...
    if dev_examples is not None:
        ...
        dev_dataset = NERDataset(
            opt.task_type, dev_features, "dev", use_type_embed=opt.use_type_embed
        )

        dev_loader = DataLoader(
            dev_dataset, batch_size=opt.eval_batch_size, shuffle=False, num_workers=0
        )
