...
def prepare_info():
    info_dict = {}
    with open(
        os.path.join(MID_DATA_DIR, f"{TASK_TYPE}_ent2id.json"), encoding="utf-8"
    ) as f:
        ent2id = json.load(f)
...
def mixed_prepare_info(mixed="crf"):
    info_dict = {}
    with open(
        os.path.join(MID_DATA_DIR, f"{mixed}_ent2id.json"), encoding="utf-8"
    ) as f:
        ent2id = json.load(f)
        ...
        with open(os.path.join(save_dir, f"{key}.ann"), "w", encoding="utf-8") as f:
            if not len(labels[key]):
                ...
                for idx, _label in enumerate(labels[key]):
                    f.write(
                        f"T{idx + 1}\t{_label[0]} {_label[1]} {_label[2]}\t{_label[3]}\n"
                    )
...
def mixed_predict():
    ...
    for i, model_path in enumerate(model_path_list):
        if i <= 4:
            ...
            model = SpanModel(
                bert_dir=MIX_BERT_DIR, num_tags=len(info_dict["id2ent"]) + 1
            )
            print(f"Load model from {model_path}")
            model, device = load_model_and_parallel(model, GPU_IDS, model_path)
            model.eval()
            labels = base_predict(
                model, device, info_dict, ensemble=False, mixed="span"
            )
